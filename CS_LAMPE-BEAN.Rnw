...
CNAME <- read.table("/Users/Lampe/GrantNo456417/MatParameterFitting/fromJimBean/1elem_SC1B_out.txt",
                    nrows = 1)
colnames(DATA.BEAN.RAW) <- c("TIME", "ZETA","ETSTAR", "SIGEQ", "SIGEQF", "OMEGA", "CAPF",
                         "EQCRATE_S", "STRESS_YY", "STRESS_XX")
DATA.BEAN.RAW[,"NAME"] <- "BEAN"

DATA.BEAN <- head(DATA.BEAN.RAW, -3) # remove rows that extend beyond test data
@
